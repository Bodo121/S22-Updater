...
final class LateActivate {
    ...
    static final String SCRIPT =
            ...
            + "su_state() { case \"$(id 2>/dev/null)\" in *uid=0*) echo granted;; *) echo denied;; esac; }\n"
            + "allow_list() {\n"
            + "  [ -f \"$ALLOW\" ] || return 0\n"
            + "  while IFS= read -r m || [ -n \"$m\" ]; do\n"
            + "    case \"$m\" in ''|\\#*) continue;; *[!A-Za-z0-9._-]*) echo \"$(date '+%H:%M:%S') ALLOW-SKIP invalid module name: $m\" >> \"$LOG\"; continue;; esac\n"
            + "    echo \"$m\"\n"
            + "  done < \"$ALLOW\"\n"
            + "}\n"
            + "cmd_probe() {\n"
            ...
            + "    case \"$line\" in *' - overlay '*) set -- $line; ov=\"$ov${ov:+,}$5\";; esac\n"
            + "  done < /proc/1/mountinfo 2>/dev/null\n"
            + "  echo \"OVERLAYS=$ov\"\n"
            + "}\n"
            + "cmd_mounts() {\n"
            + "  fail=0; count=0\n"
